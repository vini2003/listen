...
The database and audio chunks are stored in the operating system's application-data directory. Audio remains local until the user explicitly requests transcription, when a normalized copy is uploaded to pyannoteAI's temporary media storage. API keys are stored in Windows Credential Manager, macOS Keychain, or the platform keyring rather than SQLite or frontend storage.
